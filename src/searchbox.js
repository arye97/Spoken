...
async function getLanguageCountries(language) {
  
  let currentCountryInfo = await axios.get('https://restcountries.eu/rest/v2/lang/' + language);
  console.log(currentCountryInfo.data);
  return currentCountryInfo.data;
  
}

function findLanguageISO(language) {
  let i = 0;
...
const SearchBox = () => (
  <Dropdown
    ...
    text={Dropdown.text}
    onChange={(e, {value}) => {
      let isoCode = findLanguageISO(e.target.textContent);
      getLanguageCountries(isoCode);
    }}
  />
)
